...
3. Read project `AGY.md` and `MEMORY.md` if present, for domain context, estimand, methodology.
...
See `~/.agy/agents/references/code-review/checklist-categories.md` for full specs of all 11 categories: Reproducibility, Script Structure, Output Hygiene, Function Quality, Domain Correctness, Figure Quality, Data Persistence, Dependencies, Python-Specific, R-Specific, Cross-Language Verification.
...
Read `~/.agy/agents/references/code-review/persona-catalog.md` for full persona definitions and selection logic.
...
1. The text from `~/.agy/agents/references/code-review/subagent-template.md`, substituting `{persona_name}` and `{persona_content}` from the catalog.
2. The list of scripts to review.
3. The instruction to return ONLY JSON matching `~/.agy/agents/references/code-review/findings-schema.json`.

**Spawn all reviewers in parallel** — single message with multiple Task tool calls.

**Forbid-list to include in every sub-reviewer prompt** (because sub-agents do not inherit global rules — see `~/.agy/rules/subagent-prompt-discipline.md`):
...
Map each merged finding to the closest entry in `~/.agy/agents/references/code-review/quality-rubric.md` to determine the deduction. If no exact match, classify by severity tier and use the midpoint deduction.
...
- Framework: `~/.agy/skills/shared/quality-scoring.md` — severity tiers, thresholds, verdict rules.
- Rubric: `~/.agy/agents/references/code-review/quality-rubric.md` — issue-to-deduction mappings for this agent.
