...
- Edit `MEMORY.md`, `AGY.md`, or `README.md` — the main session handles project state
...
These NOT-DOs hold even when an action would seem like a natural next step after fixing the issues. Sub-agents do not inherit global rules — see `~/.agy/rules/subagent-prompt-discipline.md` § Standard Forbid-List for Write-Capable Sub-Agents.
...
You have a persistent Persistent Agent Memory directory at `~/.agy/agent-memory/fixer/`. Its contents persist across conversations.
