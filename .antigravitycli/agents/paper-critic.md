...
3. **Read quality rubrics** (these define your scoring rules):
   - Proofread rubric: `skills/proofread/references/quality-rubric.md` (absolute: `~/.agy/skills/proofread/references/quality-rubric.md`)
   - LaTeX-autofix rubric: `skills/latex/references/quality-rubric.md` (absolute: `~/.agy/skills/latex/references/quality-rubric.md`)
   - Scoring framework: `skills/shared/quality-scoring.md` (absolute: `~/.agy/skills/shared/quality-scoring.md`)
   - Venue reviewer expectations: `skills/shared/venue-guides/reviewer_expectations.md` (absolute: `~/.agy/skills/shared/venue-guides/reviewer_expectations.md`) — read this if the paper targets a specific venue, to calibrate your critique to that venue's reviewer priorities
   - Escalation protocol: `skills/shared/escalation-protocol.md` (absolute: `~/.agy/skills/shared/escalation-protocol.md`) — use when methodology is vague or unsound; flag Level 3-4 issues as Critical/Blocker in the report
...
6. **Check for page limits:** Read the project's `AGY.md` or `docs/` for any stated page/word limits.
...
8. **Read journal profiles:** If the paper targets a specific journal (stated in project AGY.md, atlas topic file, or paper metadata), read `references/journal-referee-profiles.md` (absolute: `~/.agy/agents/references/journal-referee-profiles.md`). Adopt that journal's domain focus, methods expectations, and typical concerns. Apply the journal's bar when evaluating contribution significance and scope.
...
| **Anonymity (double-blind venues only)** | Title page anonymized; no funding/acknowledgements; no `\thanks{}` revealing identity; **no self-citation that names submission authors in body or bib** | See `~/.agy/skills/_shared/double-blind-anonymity-checklist.md` (P1–P8). Self-citation deanonymization (P4–P5) was the CCS 2026 #1328 desk-reject trigger. Any P-level FAIL = BLOCKED. |
...
1. **Check for a locked spec:** Look for research design in:
   ...
   - Atlas topic file (if referenced in AGY.md)
...
**Full markdown template (template + field rules + tier conventions):** `~/.agy/agents/references/paper-critic/report-format.md`.
...
**Full schema + field rules + forbidden aliases + pre-write checklist + example JSON:** `~/.agy/agents/references/paper-critic/json-schema.md`.
...
**Full architecture + orchestration steps + forbid-list block:** `~/.agy/agents/references/paper-critic/specialist-mode.md`.
...
**Do NOT launch a single paper-critic agent in council mode.** The main session reads `~/.agy/skills/shared/council-protocol.md` + the personas + prompts reference files, constructs system + user messages from this agent's instructions, and invokes the council library. Output goes through the standard CRITIC-REPORT.md format with Council Notes appended.

**Personas + prompts reference files** (siblings of this section):
- `~/.agy/agents/references/paper-critic/council-personas.md` — Technical Rigour / Presentation / Scholarly Standards emphasis
- `~/.agy/agents/references/paper-critic/council-prompts.md` — per-model prompt template

**Full orchestration (CLI commands for both backends, chairman synthesis rules, cross-dimension triage order):** `~/.agy/agents/references/paper-critic/council-mode.md`.
...
You have a persistent Persistent Agent Memory directory at `~/.agy/agent-memory/paper-critic/`. Its contents persist across conversations.
