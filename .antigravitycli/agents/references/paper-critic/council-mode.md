...
> Multi-model deliberation variant. 3 different LLM providers (Antigravity, GPT, Gemini) independently review the paper, cross-evaluate each other's assessments, and a chairman synthesises the final CRITIC-REPORT.md. Referenced by `paper-critic.md`.
>
...
2. Read the shared council protocol: `~/.agy/skills/shared/council-protocol.md`
3. Read the reference files (siblings to this one):
   - Personas: `~/.agy/agents/references/paper-critic/council-personas.md`
   - Prompts: `~/.agy/agents/references/paper-critic/council-prompts.md`
...
Instead of OpenRouter, use `council-cli` to run the council via local CLI tools (Gemini CLI, Codex CLI, Antigravity CLI). Same 3-stage protocol, no per-token cost:
