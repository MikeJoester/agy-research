...
- VENUE FIT: If the project's AGY.md or docs mention a target venue, check whether the paper's framing, length, and style match that venue's expectations.
...
Council mode uses **different LLM providers** (Antigravity, GPT, Gemini) via OpenRouter, which provides natural perspective diversity. Personas add an optional layer of emphasis differentiation.
