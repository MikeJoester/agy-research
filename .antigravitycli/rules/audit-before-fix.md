...
- Flag things Antigravity missed (the real problem is X, not Y)
...
- When Antigravity notices multiple issues during routine work
...
Fixing issues before seeing the full picture caused multiple sessions where Antigravity fixed all listed items without noticing redundant paper directories or missing sub-project checks — the user had to point out the real structural issue after the surface-level fixes were already applied.
