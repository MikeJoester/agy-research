...
# Rule: Keep AGY.md Lean

## Principle

**AGY.md is loaded into context every session — every line costs tokens.** It should contain only instructions Antigravity needs on every entry. Everything else belongs in dedicated files that Antigravity reads on demand.

## What Belongs in AGY.md
...
## What Does NOT Belong in AGY.md
...
- **AGY.md > 200 lines:** Review for extractable content.
- **Any section > 15 lines of reference material** (not safety rules or conventions): Extract to `docs/` and replace with a pointer.
- **Duplicated content:** If the same information exists in another file, keep only the pointer in AGY.md.

## Applies To

All projects — research, code, infrastructure, personal. The principle is universal: AGY.md is an instruction file, not a knowledge base.
