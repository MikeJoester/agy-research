# Antigravity CLI for Academic Research

> This file is automatically read when you open this folder with Antigravity CLI.
...
<!-- synced from private AGY.md — do not edit manually -->
...
15 agents in `.agy/agents/`. See [`docs/components/agents.md`](docs/components/agents.md) for when to use each.

**To execute these agents using Antigravity CLI:**
When the user asks you to invoke one of these agents, you must first read the agent's markdown file in `.agy/agents/<name>.md`. Extract the `description` and the Markdown body (the `system_prompt`). Use your `define_subagent` tool to register the agent in your current session, and then use `invoke_subagent` to launch it.

## Rules (18 Auto-Loaded)

In `.agy/rules/` — these apply automatically to every session. See [`docs/components/rules.md`](docs/components/rules.md) for documentation.
...
| `lean-agy-md.md` | AGY.md is loaded into context every session — every line costs tokens. |
...
This helps me (Antigravity) pick up where we left off next time.
<!-- AFTER-SESSION:END -->
...
| `.agy/agents/` | Agent definitions (15 agents) |
| `.agy/rules/` | Auto-loaded rules (18 rules) |
