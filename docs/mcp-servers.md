...
MCP (Model Context Protocol) servers extend Antigravity with external tools and data sources. This infrastructure uses several MCP servers across Antigravity CLI and Antigravity Desktop.

## Server Inventory

| Server | Antigravity CLI | Antigravity Desktop | Type | Purpose |
|--------|:-----------:|:--------------:|------|---------|
...
| **github** | `~/.agy.json` (user) | `claude_desktop_config.json` | HTTP remote | GitHub repos, PRs, issues, code search |
...
| `~/.agy.json` (user scope) | Global (all projects) | github |
| `claude_desktop_config.json` | Antigravity Desktop | All servers above |
...
Exposes skills, agents, and the context library to Antigravity Desktop. Antigravity CLI loads these directly from `~/.agy/skills/` and `.agy/agents/`.
...
Fetches up-to-date documentation for any library or framework. Useful because Antigravity's training data has a knowledge cutoff.
...
Standard `@modelcontextprotocol/server-filesystem` — gives Antigravity Desktop read/write access to the project folder. Not needed in Antigravity CLI (which has direct file access).
...
2. **HTTP remote servers**: add to `~/.agy.json` with user scope
3. **Antigravity Desktop**: add to `~/Library/Application Support/Antigravity/claude_desktop_config.json`
